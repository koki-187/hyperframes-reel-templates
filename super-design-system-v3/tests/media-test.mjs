...
const here=path.dirname(fileURLToPath(import.meta.url)),root=path.resolve(here,'..'),repo=path.resolve(root,'..'),outDir=path.join(root,'runtime','outputs');
...
const systemFont='/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf';await fs.access(systemFont);const png=path.join(outDir,'ci-sharp-font-test.png');
...
const mp4=path.join(outDir,'ci-ffmpeg-test.mp4');
