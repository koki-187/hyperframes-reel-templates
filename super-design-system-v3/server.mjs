...
async function executeJob(job){job.status='processing';job.startedAt=new Date().toISOString();await writeJob(job);try{
 if(job.mode==='prompt_only')job.result={recipe:job.recipe};
  ...
  const font=await loadJapaneseFont(fontDir,{required:false});const svg=renderSocialSvg(job.recipe,job.input,font||{}),svgFile=`${job.id}.svg`;await fs.writeFile(path.join(outputDir,svgFile),svg);job.result={files:[fileResult(svgFile,'image/svg+xml')],font:font?{name:font.name,sha256:font.sha256}:null};
 }else if(job.mode==='template_png'){
  const font=await loadJapaneseFont(fontDir,{required:process.env.DNA_ALLOW_FONT_FALLBACK!=='true'});if(!font&&process.env.DNA_ALLOW_FONT_FALLBACK==='true'){const svg=renderSocialSvg(job.recipe,job.input,{}),pngFile=`${job.id}.png`;const sharp=(await import('sharp')).default;await sharp(Buffer.from(svg)).png().toFile(path.join(outputDir,pngFile));job.result={files:[fileResult(pngFile,'image/png')],font:null,warning:'FONT_FALLBACK_USED'}}else{const pngFile=`${job.id}.png`;const result=await renderSocialPng({recipe:job.recipe,input:job.input,font,output:path.join(outputDir,pngFile)});job.result={files:[fileResult(pngFile,'image/png')],font:result.font,layout:result.layout}}
...
app.get('/api/health',(_,res)=>res.json({ok:true,name:'14DNA-ENGINE',version:'4.3.1',freeMode:true,modes:MODES}));app.get('/api/catalog',(_,res)=>res.json({motion:MOTION,formats:FORMATS,modes:MODES}));app.get('/api/fonts',requireToken,async(_,res)=>res.json(await inspectFonts(fontDir)));app.post('/api/recipe',(req,res)=>{try{res.json(createRecipe(req.body||{}))}catch(e){res.status(400).json({error:e.message})}});
