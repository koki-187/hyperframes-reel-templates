...
export function renderSocialSvg(recipe,input={},font={}){
  const format=resolveFormat(recipe,input),{width,height}=format,vertical=height>width,margin=Math.round(width*.075);
  ...
  const lineHeight=Math.round(width*(vertical?.105:.064)),titleSize=Math.round(width*(vertical?.092:.064)),startY=Math.round(height*.27);
  const title=layout.lines.map((line,index)=>`<text x="${margin}" y="${startY+index*lineHeight}" class="headline">${esc(line)}</text>`).join('');
  ...
  const meta=esc(JSON.stringify({format:format.id,total:layout.total,warnings:layout.warnings,font:font.name||'fallback'}));
